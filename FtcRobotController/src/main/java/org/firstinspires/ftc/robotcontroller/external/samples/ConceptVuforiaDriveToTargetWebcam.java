...
/**
 * This OpMode illustrates using a webcam to locate and drive towards ANY Vuforia target. The code
 * assumes a basic two-wheel Robot Configuration with motors named left_drive and right_drive. The
 * motor directions must be set so a positive drive goes forward and a positive turn rotates to the
 * right.
 *
 * <p>Under manual control, the left stick will move forward/back, and the right stick will turn
 * left/right. This is called POV Joystick mode, different than Tank Drive (where each joystick
 * controls a wheel). Manually drive the robot until it displays Target data on the Driver Station.
 * Press and hold the *Left Bumper* to enable the automatic "Drive to target" mode. Release the Left
 * Bumper to return to manual driving mode.
 *
 * <p>Use DESIRED_DISTANCE to set how close you want the robot to get to the target. Speed and Turn
 * sensitivity can be adjusted using the SPEED_GAIN and TURN_GAIN constants.
 *
 * <p>For more Vuforia details, or to adapt this OpMode for a phone camera, view the
 ...
 * list.
 *
 * <p>IMPORTANT: In order to use this OpMode, you need to obtain your own Vuforia license key as is
 * explained below.
 */
...
public class ConceptVuforiaDriveToTargetWebcam extends LinearOpMode {
  ...
  final double DESIRED_DISTANCE =
      8.0; //  this is how close the camera should get to the target (inches)
  //  The GAIN constants set the relationship between the measured position error,
  //  and how much power is applied to the drive motors.  Drive = Error * Gain
  //  Make these values smaller for smoother control.
  final double SPEED_GAIN =
      0.02; //  Speed Control "Gain". eg: Ramp up to 50% power at a 25 inch error.   (0.50 / 25.0)
  ...
  public void runOpMode() {
    ...
    double targetBearing =
        0; // Robot Heading, relative to target.  Positive degrees means target is to the right.
